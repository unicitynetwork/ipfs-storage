...
configure_nginx() {
    ...
    # Include alias server blocks generated by ssl-manager (if any)
    if [ -f /tmp/.ssl-alias-nginx.conf ] && [ -s /tmp/.ssl-alias-nginx.conf ]; then
        echo "[entrypoint] Including ssl-manager alias nginx blocks"
        # Insert before the last closing brace of http{}
        local last_brace
        last_brace=$(grep -n "^}" /etc/nginx/nginx.conf | tail -1 | cut -d: -f1)
        if [ -n "$last_brace" ]; then
            head -n $((last_brace - 1)) /etc/nginx/nginx.conf > /tmp/nginx-merged.conf
            cat /tmp/.ssl-alias-nginx.conf >> /tmp/nginx-merged.conf
            echo "}" >> /tmp/nginx-merged.conf
            cp /tmp/nginx-merged.conf /etc/nginx/nginx.conf
            rm -f /tmp/nginx-merged.conf
        fi
    fi
...
# Step 5: Configure nginx (includes alias blocks from ssl-manager if present)
configure_nginx
